...
delta_He_vals = [ -0.1, 0.2, 0.5, 0.8  ][::-1]
delta_H_vals = [ -0.3, -0.1, 0.1, 0.3  ][::-1]
...
for i in range(nrows):
  for j in range(ncols):
    ...
    if j < 2: 
      delta_vals = delta_H_vals
      label_base = r'$\Delta z_\mathrm{H}\,=\,$' 
    else:     
      delta_vals = delta_He_vals
      label_base = r'$\Delta z_\mathrm{He}\,=\,$'
      
    
    for indx,delta in enumerate(delta_vals):
      label = label_base + r'${0:.1f}$'.format( delta)
      color = colors[indx]
      ax.plot( z+delta, rate, label=label, color=color )
...
figure_name = output_dir + 'uvb_rates_delta.png'
