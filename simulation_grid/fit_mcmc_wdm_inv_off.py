...
ps_range = SG.Get_Power_Spectrum_Range( kmin=kmin, kmax=kmax )
sim_ids = SG.sim_ids
...
data_covariance = { 'P(k)': { 'type': { 'Boera':'local',  }, 'sigma_factor': { 'Boera':1.0 }, 'cross_elements_factor': { 'Boera':1.0, } } }
...
comparable_data = Get_Comparable_Composite( fields_to_fit, z_min, z_max, ps_parameters=ps_parameters, log_ps=False, 
                                            systematic_uncertainties=data_systematic_uncertainties, no_use_delta_p=no_use_delta_p,
...
if elapsed_time < 60: print(f'Elapsed time: {elapsed_time} secs')
elif elapsed_time < 3600: print(f'Elapsed time: {elapsed_time/60} min')
else: print(f'Elapsed time: {elapsed_time/3600} hrs')
