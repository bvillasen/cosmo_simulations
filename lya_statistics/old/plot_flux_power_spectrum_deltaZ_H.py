...
for model_id in range(n_models):
  input_dir = root_dir + f'sim_{model_id}/analysis_files/'
  data_sim = {}
  factor = 1.05
  for n_file in range(n_files):
    ...
    data_sim[n_file] = { 'z':z, 'k_vals':k_vals[indices], 'ps_mean':delta_ps[indices] } 
...
for i in range(3):
  ...
  for model_id in range(n_models):
    ...
    if diff_min < 1e-1:
      ...
      label = r'$\Delta z_\mathrm{H} =$' + f' {deltaZ_H_vals[model_id]:0.1f}'
      ps_ref = data_ps[ref_id][snap_index]['ps_mean']
      if rescaled:
        factor = ps_ref[0] / ps_mean[0]
        ps_mean *= factor
      
      ps_diff = ( ps_mean - ps_ref ) / ps_ref 
      
      ax1.plot( k_vals, ps_mean, c=color, ls=ls, label=label )
      
      ax2.plot( k_vals, ps_diff, c=color, ls=ls  )
...
figure_name = output_dir + 'flux_ps_deltaZ_H.png'
if rescaled: figure_name = output_dir + 'flux_ps_deltaZ_H_rescaled.png'
