...
pos_label_y = [ 0.5, 0.5, 0.5, 0.5 ]
pos_label_x = [ 0.85, 0.85, 0.1, 0.1 ]
...
for i in range(nrows):
  for j in range(ncols):
    ...
    ax.text( pos_label_x[fig_id], pos_label_y[fig_id], r'$z=${0:.1f}'.format(np.round(z)), horizontalalignment='center',  verticalalignment='center', transform=ax.transAxes, fontsize=figure_text_size, color=text_color) 
    ...
    ax.set_ylabel( r' $f\,(F)$', fontsize=label_size, color= text_color )
    ax.set_xlabel( r'$ F$', fontsize=label_size, color= text_color )
...
figure_name = output_dir + 'flux_distribution.png'
