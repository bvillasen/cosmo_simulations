...
z_range_vals = [  [5.5, 5.6], [5.6, 5.7], [5.7, 5.8], [5.8, 5.9], [5.9, 6.0], [6.0, 6.1], [6.1, 6.2]  ]
n_zrange = len( z_range_vals  )

z_range_indx = 7
# for z_range_indx in range(n_zrange):
z_range = z_range_vals[z_range_indx]

zmin, zmax = z_range
indices = ( trace >= zmin ) * ( trace <= zmax ) 

hpi_sum = 0.95 

file_name = input_dir + 'samples_power_spectrum.pkl'
ps_samples_all = Load_Pickle_Directory( file_name )
ps_samples_selected = {}
for z_id in ps_samples_all:
  ps_samples = ps_samples_all[z_id]
  z = ps_samples['z']
  trace = ps_samples['trace']
  samples_selected = trace[indices]
  samples = samples_selected.T
  ...
  ps_samples_selected[z_id] = {}
  ps_samples_selected[z_id]['z'] = z
  ps_samples_selected[z_id]['Highest_Likelihood'] = ps_mean
  ps_samples_selected[z_id]['sigma'] = ps_sigma
  ps_samples_selected[z_id]['k_vals'] = ps_samples['k_vals']
  ps_samples_selected[z_id]['lower'] = ps_lower
  ps_samples_selected[z_id]['higher'] = ps_higher

ps_samples_selected['label'] = f'$z=$[{zmin}, {zmax}]'
...
data_sets_sim[1] = ps_samples_selected
...
fileName = output_dir + f'flux_ps_difference_boera_{z_range_indx}.png'
