...
# Load reference temeperatures
reference_sim = 3
sim_name = sim_names[reference_sim]
sim_dir = base_dir + sim_name + '/'
fit_dir = sim_dir + 'analysis_files/fit_mcmc_delta_0_1.0/'
reference_temps = {}
for snap_id in snap_ids:
  file_name = fit_dir + f'fit_{snap_id}.pkl'
  fit_data = Load_Pickle_Directory( file_name, 'r' )
  T0 = 10**fit_data['T0']['mean']
  reference_temps[snap_id] = T0
...
for indx in local_indices:  
  ...
  for snap_id in snap_ids:
    ...
    Tref = reference_temps[snap_id]
    rescale_factor = Tref / T0
    print( f'{sim_name}   T0:{T0:.2e}  Tref:{Tref:.2e}  rescale_factor:{rescale_factor:.2f}  ')
    ...
    skewers_data['temperature'] *= rescale_factor
    
    out_file_name = output_dir + f'lya_flux_{space}_{snap_id:03}_rescaled_T0.h5'
