...
refference_id = np.where( param_vals == 0.0 )[0][0]
pk_reference_all = pk_data['pk_samples'][refference_id]['simulation']


file_name = input_dir + 'pk_interpolated.pkl'
pk_data = Load_Pickle_Directory( file_name )
param_vals = np.array(pk_data['param_change_vals'])
refference_id = np.where( param_vals < 1e-5 )[0][0]
print( f'CDM id: {refference_id}')
...
for i in range(3):
  ...
  for data_id in ids_to_plot:
    ...
    val = 1/param_vals[data_id]
    label = r'$m_\mathrm{WDM}$=' + f'{val:.1f}'
    ax.plot( k_vals, pk_diff, label=label )
    
  ax.axhline( y=0, ls='--', c='C3')
    
  ax.legend( loc=3, frameon=False, fontsize=font_size-6)
  ...
  ax.set_xscale('log')
  ax.set_ylabel( r'$ \Delta P\,(k) / P_{\mathrm{CDM}}\,(k)$', fontsize=label_size, color= text_color ) 
...
figure_name = output_dir + 'pk_convergence_interpolated.png'
