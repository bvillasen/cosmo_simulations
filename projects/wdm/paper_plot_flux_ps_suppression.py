...
for data_id, data_name in enumerate(data_names):
  ...
  data_wdm[sim_id] = { 'k_vals':k_vals, 'ps_ratio':ps_ratio } 
  sim_id += 1




# alpha_vals = [ 1.0, 1.6, 1.4, 1.2, 0.8, 0.6, 0.4 ]
...
# labels_alpha = [ r'$\alpha\,=\,1.6$', r'$\alpha\,=\,1.4$', r'$\alpha\,=\,1.2$', r'$\alpha\,=\,0.8$', r'$\alpha\,=\,0.6$', ]
...
for data_id, data_name in enumerate(data_names):
  ...
  data_alpha[sim_id] = { 'k_vals':k_vals, 'ps_ratio':ps_ratio, 'T0':T0 } 
  sim_id += 1
...
for data_id, data_name in enumerate(data_names):
  ...
  file.close()
  ps_ratio = ps_mean / ps_ref
  data_delta_z[sim_id] = { 'k_vals':k_vals, 'ps_ratio':ps_ratio } 
  sim_id += 1
...
y_labels = [r'$ P\,(k) / P_0\,(k)$', r'$ P\,(k) / P_0\,(k)$', r'$ P\,(k) / P_0\,(k)$' ]

line_width = 2.0

x_range = [3e-3, 3e-1]
y_range = [ [0.35, 1.05], [0.55, 1.33], [0.92, 1.11], ]
...
for i in range(ncols):
  ...
  for data_id in data:
    ...
    ps_ratio = data[data_id]['ps_ratio']  
    ps_ratio /= ps_ratio[0]
