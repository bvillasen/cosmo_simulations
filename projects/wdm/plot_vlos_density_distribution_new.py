...
field_list = [  'density', 'los_velocity',  ]
...
density = np.log10(skewer_dataset['density'].flatten() / cosmo.rho_gas_mean)
los_velocity = skewer_dataset['los_velocity'].flatten() 

n_points = 200
dens_vals = np.linspace( -2, 5, n_points )
...
phase, yedges, xedges  = np.histogram2d( density, los_velocity, bins=[dens_vals, vel_vals] )
phase = phase.T
phase = phase.astype(np.float)
phase /= phase.sum()
vel_centers = (xedges[:-1] + xedges[1:])/2
dens_centers = (yedges[:-1] + yedges[1:])/2
# 
# indices = phase > 0
...
im = ax.imshow( phase, cmap=colormap, extent=[dens_centers.min(), dens_centers.max(), vel_centers.min(), vel_centers.max(), ], origin='lower' )
...
cb.set_label( r'$ \log_\mathrm{10} \, PDF( \rho / \bar{\rho}, \, v_\mathregular{pec})$', fontsize=18)
...
ax.set_xlabel( r'$\log_{10} (\rho / \bar{\rho}) $ ', fontsize=label_size, color=text_color )

ax.set_aspect(1/70)

if logfigure_name = output_dir + f'vlos_density_distribution_{n_file}_log.png'
