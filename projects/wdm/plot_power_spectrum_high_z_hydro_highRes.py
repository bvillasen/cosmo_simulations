...
output_dir = proj_dir + 'figures/pk_high_z_high_res/'
...
for sim_id, sim_name in enumerate(sim_names):
  ...
  for snap_id in range(6):
    input_dir = base_dir + f'{sim_base_name}_{sim_name}/{density_type}/power_spectrum_files/'
...
data_hydro_hr = {}
for sim_id, sim_name in enumerate(sim_names):
  data_hydro_hr[sim_id] = {}
  for snap_id in range(6):
    input_dir = base_dir + f'{sim_base_name}_{sim_name}/{density_type}/power_spectrum_files/'
    file_name = input_dir + f'power_spectrum_{data_type}_{snap_id}.pkl'
    data = Load_Pickle_Directory( file_name )
    data_hydro_hr[sim_id][snap_id] = data

data_type = 'particles'
data_particles_hr = {}
for sim_id, sim_name in enumerate(sim_names):
  data_particles_hr[sim_id] = {}
  for snap_id in range(6):
    input_dir = base_dir + f'{sim_base_name}_{sim_name}/{density_type}/power_spectrum_files/'
    file_name = input_dir + f'power_spectrum_{data_type}_{snap_id}.pkl'
    data = Load_Pickle_Directory( file_name )
    data_particles_hr[sim_id][snap_id] = data
...
for i in range(2):
  if i == 0: data = data_particles
  if i == 1: data = data_hydro
  ...
  for i in range(6):
    ...
    pk_0 = data[sim_id][id]['power_spectrum']*dx
    if mult_k3: pk_0 *= k_0**3
    ...
    sim_id = 1
    z = data[sim_id][id]['z']
    k_1  = data[sim_id][id]['k_vals']
    pk_1 = data[sim_id][id]['power_spectrum']*dx
    c = colors[i]
    if mult_k3: pk_1 *= k_1**3
    ax1.plot( k_1, pk_1, ls='--', c=c )

    diff = pk_1 / pk_0
    ax2.plot( k_0, diff, ls='--', c=c )

for i in range(2):
  if i == 0: data = data_particles_hr
  if i == 1: data = data_hydro_hr
  ...
  for i in range(6):
    ...
    c = colors[i]
    if mult_k3: pk_0 *= k_0**3
    ax1.plot( k_0, pk_0, label='', ls="dashdot", c=c )
    ...
    c = colors[i]
    if mult_k3: pk_1 *= k_1**3
    ax1.plot( k_1, pk_1, ls='dotted', c=c )

    diff = pk_1 / pk_0
    ax2.plot( k_0, diff, ls='dotted', c=c )
...
figure_name = output_dir + f'flux_ps_{n_points}_hydro_log.png'
if mult_k3: figure_name = output_dir + f'flux_ps_{n_points}_hydro_log_k3.png'
