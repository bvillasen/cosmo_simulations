...
sim_dir = data_dir + 'cosmo_sims/256_50Mpc/'
input_dir_0 = sim_dir + 'analysis_files_lya_cpu_new/'
input_dir_1 = sim_dir + 'analysis_files/'
...
for snap_id in range( n_files ):
  ps_0 = data_all[0][snap_id]['ps_mean']
  ps_1 = data_all[1][snap_id]['ps_mean']
  diff = ps_1 / ps_0 - 1
  # print( f'Snap: {snap_id}  diff: {diff}' )
  print( f'Snap: {snap_id}  diff   min: {diff.min()}   max: {diff.max()}' )
